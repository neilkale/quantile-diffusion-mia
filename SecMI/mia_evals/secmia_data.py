...
def secmi_attack_data(model, FLAGS, dataset_root, timestep=10, t_sec=100, batch_size=128, dataset='cifar10'):
    # load splits
    ...
    member_scores, nonmember_scores = naive_statistic_attack(t_results, metric='l2')
    t_results['member_scores'] = member_scores
    t_results['nonmember_scores'] = nonmember_scores
...
if __name__ == '__main__':
    ...
    parser.add_argument('--t_sec', type=bool, default=50)
    ...
    if not os.path.exists(f'quantile_datasets/secmi_reconstructed_images_and_t_errors_{dataset_name}_dt{args.k}_t{args.t_sec}.pt'):
        t_results = secmi_attack_data(model, FLAGS, dataset_root=args.data_root, t_sec=args.t_sec, timestep=args.k, batch_size=1024, dataset=args.dataset)
        torch.save(t_results, f'quantile_datasets/secmi_reconstructed_images_and_t_errors_{dataset_name}_dt{args.k}_t{args.t_sec}.pt')
    else:
        t_results = torch.load(f'quantile_datasets/secmi_reconstructed_images_and_t_errors_{dataset_name}_dt{args.k}_t{args.t_sec}.pt')

    original_images = torch.cat([t_results['member_diffusions'], t_results['nonmember_diffusions']]).cpu()
    reconstructed_images = torch.concat([t_results['member_internal_samples'], t_results['nonmember_internal_samples']]).cpu()
    t_errors = torch.cat([t_results['member_scores'], t_results['nonmember_scores']]).cpu()
    labels = torch.cat([torch.ones(t_results['member_diffusions'].size(0)), torch.zeros(t_results['nonmember_diffusions'].size(0))]).cpu()
    labels = ['member' if label == 1 else 'nonmember' for label in labels]
    dataset = QuantileRegressionDataset(dataset_name, original_images=original_images, reconstructed_images=reconstructed_images, t_errors=t_errors, labels=labels, augment=False)
    dataset.save(f'quantile_datasets/secmi_combined_dataset_{dataset_name}_dt{args.k}_t{args.t_sec}.pt')
    print(f'Saved dataset to secmi_combined_dataset_{dataset_name}_dt{args.k}_t{args.t_sec}.pt')

    # Save the train/eval split
    nonmember_eval_split = 0.2
    num_members = len(t_results['member_scores'])
    num_nonmembers = len(t_results['nonmember_scores'])

    nonmember_indices = np.arange(num_members, num_members + num_nonmembers)
    np.random.shuffle(nonmember_indices)
    split_idx = int(num_nonmembers * nonmember_eval_split)
    quantile_eval_indices = np.concatenate((np.arange(0, num_members), nonmember_indices[:split_idx]))
    quantile_train_indices = nonmember_indices[split_idx:]

    output_path = f"quantile_datasets/secmi_{dataset_name}_dt{args.k}_t{args.t_sec}_quantile_split.npz"
